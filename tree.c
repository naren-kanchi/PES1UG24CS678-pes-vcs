...
// Recursive helper: given a sorted array of index entries, build and write tree
// objects for a given directory prefix (depth level).
//
// entries: all index entries that share the current prefix
// count: number of such entries
// prefix: the current directory prefix (e.g., "src/" for entries under src/)
// id_out: output for the root tree hash of this level
static int write_tree_level(IndexEntry *entries, int count, const char *prefix, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        // Strip the prefix from the path to get the relative name
        const char *rel = path + prefix_len;

        // Check if there's a '/' — meaning this entry is in a subdirectory
        ...
        if (slash == NULL) {
            ...
            // Get the subdirectory name (portion before the slash)
            ...
            // Add a directory entry for this subtree
            TreeEntry *te = &tree.entries[tree.count++];
            te->mode = MODE_DIR;
            te->hash = sub_id;
